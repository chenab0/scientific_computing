...
u_cap = fft2(u)
v_cap = fft2(v)
y_flat = np.hstack([u_cap.flatten(), v_cap.flatten()])
...
def spc_rhs(t, y_flat):
  ...
  u_cap_flat = y_flat[:n**2]
  v_cap_flat = y_flat[n**2:]
  ...
  u_t = fft2(f) - D1 * K * u_cap
  v_t = fft2(g) - D2 * K * v_cap

  return np.hstack([u_t.flatten(), v_t.flatten()])
...
y_cheb =  x_cheb #Same for Y
X, Y = np.meshgrid(x_cheb, y_cheb)

#Computing Laplacian

I = np.eye(len(D2))
Laplacian = np.kron(I, D2) + np.kron(D2, I) #2D Laplacian
...
u_flat = u.flatten()
v_flat = v.flatten()
y_flat = np.hstack([u_flat, v_flat])
